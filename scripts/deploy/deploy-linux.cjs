/**
 * Docgen - Release-Based Deployment Script (Linux/WSL)
 *
...
const CONFIG = {
    ...
    paths: {
        ...
        basePath: (process.env.SERVER_BASE_PATH || process.env.SERVER_DEST_PATH || '/var/www/docgen').trim(),
...
async function deploy(options = {}) {
    ...
    try {
        ...
        await execSSH(ssh, 'sudo supervisorctl restart docgen-horizon:*', { ignoreError: true });
        log('✅', 'Queue workers restarted');

        log('♻️', ' Restarting Reverb WebSocket server...');
        await execSSH(ssh, 'sudo supervisorctl restart docgen-reverb', { ignoreError: true });
        ...
        log('🌐', 'Site: https://api.docgen.philiprehberger.com');
...
if (args.includes('--help') || args.includes('-h')) {
    console.log(`
Docgen - Release-Based Deployment (Linux/WSL)
