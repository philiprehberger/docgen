...
export type SdkSnippets = {
  ...
  install: string;
  sourceUrl: string;
  /** Full quickstart code block — template create → freeze → render → poll. */
  quickstart: string;
  quickstartLang: string;
  /** Stand-alone pollRender helper snippet. */
  poll: string;
  pollLang: string;
...
export function SdkPage({ snippets }: { snippets: SdkSnippets }) {
  return (
    <div className="mx-auto max-w-3xl px-6 py-16">
      <p className="text-xs uppercase tracking-widest text-amber-300/80 mb-3">{snippets.lang} SDK</p>
      <h1 className="text-3xl sm:text-4xl font-semibold tracking-tight mb-2">{snippets.pkg}</h1>
      <p className="text-zinc-400 mb-10">
        Generated client + hand-tuned <code className="text-amber-300/90 text-sm">pollRender</code> helper.{" "}
        <Link href={snippets.sourceUrl} className="text-amber-300 hover:underline underline-offset-4">
      ...
      <h2 className="text-xl font-semibold mt-10 mb-3">Quickstart</h2>
      <p className="text-sm text-zinc-400 mb-3">
        Author a template, freeze a version, submit a render, poll until terminal.
      </p>
      <CodeBlock language={snippets.quickstartLang}>{snippets.quickstart}</CodeBlock>

      <h2 className="text-xl font-semibold mt-10 mb-3">The <code className="text-amber-300/90 text-base">pollRender</code> helper</h2>
      <p className="text-sm text-zinc-400 mb-3">
        Exponential backoff (500ms → 5s ceiling), configurable wall-clock budget.
      </p>
      <CodeBlock language={snippets.pollLang}>{snippets.poll}</CodeBlock>
